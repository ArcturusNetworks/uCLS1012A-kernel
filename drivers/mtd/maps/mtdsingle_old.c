/***************************************************************************
 *   Copyright (c) 2008-2020 Arcturus Networks Inc.
...
static int __init mtdsingle_init(void)
{
	if (fsize < MIN_SECTOR_SIZE) {
		pr_err("%s: Flash Size (fsize=xxx) MUST be set.\n",
		       __FILE__);
		pr_info("%s: Arguments: fbase, fsize, fbus, pbase, psize, bname. (See file for details)\n",
		       __FILE__);
		return -1;
	}
