...
#define LS1012A_FRDM            0 
// FIXME: needs a way to enable it - we need it due to 3V3 RESER pin (output) needs to stay HIGH but
// read back only shows LOW so we have to write 1 every time. This fix is for GPIO1 ports - the driver
// here will not work on GPIO2 ports 
#define RESET_3V3_PIN           23
...
#if LS1012A_FRDM
	//pr_info("%s outdata=[0x%x] pin=[%u] val=[%d]", __func__, (mpc8xxx_gc->data | mpc8xxx_gpio2mask(RESET_3V3_PIN)), gpio, val);
	gpio_out32((mpc8xxx_gc->data | mpc8xxx_gpio2mask(RESET_3V3_PIN)), mm->regs + GPIO_DAT);
#else
	gpio_out32(mpc8xxx_gc->data, mm->regs + GPIO_DAT);
#endif
	//pr_info("%s in data=[0x%x]", __func__, gpio_in32(mm->regs + GPIO_DAT));
