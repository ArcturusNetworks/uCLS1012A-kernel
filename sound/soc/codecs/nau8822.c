 ...
 *
 * Based on WM8974.c
 * 2019.04 - Backported to linux-4.9.x
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < params->max / sizeof(u16); i++) {
		regmap_read(nau8822->regmap, reg + i, &reg_val);
				 ...
				 unsigned int freq, int dir)
{
	...
	nau8822->sysclk = freq;
	dev_dbg(codec->dev, "master sysclk %dHz, source %s\n", freq,
...
static int nau8822_config_clkdiv(struct snd_soc_dai *dai, int div, int rate)
{
	...
	case NAU8822_CLK_MCLK:
		...
		dev_dbg(codec->dev, "master clock prescaler %x for fs %d\n",
			div, rate);

		/* master clock from MCLK and disable PLL */
		regmap_update_bits(nau8822->regmap, NAU8822_REG_CLOCKING,
			NAU8822_MCLKSEL_MASK, (div << NAU8822_MCLKSEL_SFT));
		regmap_update_bits(nau8822->regmap, NAU8822_REG_CLOCKING,
			NAU8822_CLKM_MASK, NAU8822_CLKM_MCLK);
